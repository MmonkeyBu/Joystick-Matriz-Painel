...
void menu_handle_button_a(int x, int y, bool estado_inverter) {
    if (exibindo_funcao) {
        ...
        // Limpa a matriz de LEDs ao sair da função de ajuste de brilho
        if (!is_joysticks_selected && selected_option == 2) { // Verifica se está saindo do "Ajustar Brilho"
            exibir_frame(f_01, cor, pio, sm, 0.0); // Exibe um frame vazio (todos os LEDs apagados)
        }

        if (is_joysticks_selected) {
            menu_draw(menu_options_joysticks, NUM_OPTIONS_SUB_1); // Redesenha o submenu de joysticks
        } else {
            menu_draw(menu_options_matrix, NUM_OPTIONS_SUB_2);    // Redesenha o submenu da matriz
        }
        return;
    }
    ...
    if (in_sub_menu) {
        ...
        if ((is_joysticks_selected && strcmp(menu_options_joysticks[selected_option], "Voltar") == 0) ||
            ...
            // Limpa a matriz de LEDs ao voltar ao menu principal
            exibir_frame(f_01, cor, pio, sm, 0.0); // Exibe um frame vazio (todos os LEDs apagados)
